...
// downsample   - if true then YCbCr 4:2:0 format is used (smaller size, minor quality loss) instead of 4:4:4, not
// relevant for grayscale comment      - optional JPEG comment (0/NULL if no comment), must not contain ASCII code 0xFF
...
// ). However, his code documentation is quite sparse - probably because it wasn't written from scratch and is (quote:)
// "based on a javascript jpeg writer", most likely Andreas Ritter's code:
// https://github.com/eugeneware/jpeg-js/blob/master/lib/encoder.js
//
// Therefore I wrote the whole lib from scratch and tried hard to add tons of comments to my code, especially describing
// where all those magic numbers come from. And I managed to remove the need for any external includes ... yes, that's
// right: my library has no (!) includes at all, not even #include <stdlib.h> Depending on your callback WRITE_ONE_BYTE,
// the library writes either to disk, or in-memory, or wherever you wish. Moreover, no dynamic memory allocations are
// performed, just a few bytes on the stack.
//
...
// b) YCbCr 4:2:0 downsampling is often about 20% more efficient (=smaller) than the default YCbCr 4:4:4 with only
// little visual loss
//
